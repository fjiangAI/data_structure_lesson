...
1. **找到前驱**：要在 10 后插入 20，必须先保存前驱结点 p=10。
2. **分配新结点**：创建新结点 s。此时先让 s->next 指向原来的 p->next，也就是 30。
3. **接入链表**：再令 p->next = s。顺序不能反，否则会丢失 30 之后的链。
4. **删除结点的本质**：删除 20 时，让前驱 10 的 next 越过 20 指向 30，然后释放 20。
