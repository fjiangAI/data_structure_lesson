...
function demoFor(week) {
  ...
  return { ...common, ...demo, pseudocode: pseudocodeFor(week) };
}
...
- 每个交互演示页包含“动画步骤对应伪代码”面板，帮助学生把结构变化映射到实现思路。
...
\`assets/\` 存放公共可视化样式和脚本。\`test/\` 包含随堂测试、课后作业、LLM/代码大模型辅助学习任务和参考答案。\`assignments/\` 是跨周实验作业模板。\`teacher_guide/\` 是教师和助教使用指南。\`onlineweb/\` 是课程总网站，用于集中浏览周次、查看知识图谱、做练习、阅读材料和记录学习进度。\`.github/workflows/pages.yml\` 用于自动部署 GitHub Pages，\`.github/workflows/ci.yml\` 用于课程结构检查和 C 示例编译，\`tools/generate_course.mjs\` 用于重新生成课程材料。
...
第一个脚本检查 16 个 week 文件夹、交互演示数据、阅读器、实验模板和教师指南是否完整。第二个脚本会用 \`gcc\`、\`clang\` 或 \`CC\` 环境变量指定的编译器编译所有 C 示例。
...
@media (max-width: 920px) {
  .week-hero-grid, .stage-grid, .code-sync-panel, .traversal-layout, .search-panels { grid-template-columns: 1fr; }
    ...
    steps:
      ...
      - name: Prepare static site
        ...
        run: |
          rm -rf _site
          mkdir -p _site
          cp -R index.html README.md AI_LEARNING_GUIDE.md CONTRIBUTING.md Makefile .editorconfig .gitattributes assets onlineweb test assignments teacher_guide tools week* _site/
