...
function operationTable(week) {
  if (week.id === 16) {
    return [
      "| 环节 | 关键动作 | 产出物 |",
      "|---|---|---|",
      ...week.operations.map((r) => `| ${r[0]} | ${r[1]} | ${r[2]} |`)
    ].join("\n");
  }
  return markdownTable(week.operations);
}

function operationTeachingLine(week, op, index) {
  if (week.id === 16) {
    return `${index + 1}. **${op[0]}**：${op[1]}。这不是单个算法的复杂度结论，而是综合设计的评审环节，要求形成 ${op[2]}。`;
  }
  if (op[2] === "概念判断") {
    return `${index + 1}. **${op[0]}**：${op[1]}。这是性质判断题，要用相等关键字、移动方式或结构不变量来证明。`;
  }
  return `${index + 1}. **${op[0]}**：${op[1]}。分析时先找会重复执行的语句，再判断重复次数，所以代价为 ${op[2]}。`;
}

function studyGuideFor(week) {
  const demo = demoFor(week);
  const opLines = week.operations.map((op, i) => operationTeachingLine(week, op, i)).join("\n");
...
function weeklyTeachingData(week) {
  const data = {
    1: {
      focus: "用同一段代码分别数顺序、循环、二分和递归的执行次数",
      outcome: "能把一段 C 代码转化为增长函数，并说明输入规模是什么",
      deepDive: [
        "给定 n=8，顺序求和执行 8 次加法，二分查找最多比较 4 次，双重循环比较 28 对元素。",
        "让学生把 n 改为 16，再重新数次数：线性翻倍，对数只多 1 层，平方级增长明显加快。",
        "最后追问：如果函数里有递归，输入规模是 n、n/2 还是调用次数？这一步为后续树、图和排序递归打基础。"
      ],
      checkpoints: ["指出 `linear_sum` 的基本操作", "解释 `binary_search` 为什么每轮规模减半", "给出 `O(n^2)` 的最小代码形态", "说明递归出口为什么也是复杂度分析的一部分"],
      misconceptions: ["把运行时间和复杂度完全等同", "只写 O(n)，不说明 n 是什么", "看到两层循环就机械写 O(n^2)，不看循环范围", "忽略递归出口"],
      exercises: ["把 `pair_count_less_than` 的比较次数写成 n(n-1)/2", "设计一个 O(n log n) 的代码框架，不要求完整算法", "让 LLM 判断复杂度，并指出它是否说明了输入规模"],
      quiz: ["给出 `for(i=0;i<n;i++)` 的基本操作和次数。", "二分查找在 32 个元素中最多比较几次？", "为什么 `O(1)` 不等于程序只执行 1 条语句？", "写出一个递归没有出口的风险。", "让模型解释复杂度时必须追问哪一个前提？"]
    },
    2: {
      focus: "顺序表中间插入时从后往前移动元素",
      outcome: "能手推顺序表插入/删除的移动方向，并解释 `size/capacity` 不变量",
      deepDive: [
        "用数组 [10,20,30,40,_,_] 在下标 2 插入 25。",
        "先移动 40 到下标 4，再移动 30 到下标 3，最后写入 25 并让 size 从 4 变为 5。",
        "故意让学生从前往后移动一次，观察 30 被覆盖，从而理解移动方向不是代码细节，而是不变量要求。"
      ],
      checkpoints: ["说明 `size` 和 `capacity` 的区别", "画出插入前后有效区间", "解释为什么中间插入是 O(n)", "给出扩容失败时函数应如何返回"],
      misconceptions: ["把 capacity 当成已有元素个数", "插入移动方向写反", "删除后忘记 size--", "扩容后旧指针处理不清"],
      exercises: ["手推在下标 0、size、非法下标插入的结果", "补一个扩容失败测试", "比较顺序表和链表在按下标访问上的差别"],
      quiz: ["顺序表下标 2 插入前要移动哪些元素？", "`size == capacity` 时插入应先做什么？", "删除下标 1 后哪些元素需要前移？", "按下标访问为什么是 O(1)？", "LLM 生成顺序表代码时最需要检查哪个边界？"]
    },
    3: {
      focus: "链表插入的两条 `next` 赋值顺序",
      outcome: "能画出链表插入/删除前后箭头，并判断是否断链或丢失后继",
      deepDive: [
        "在 10 -> 30 中插入 20，先写 `s->next = p->next`，再写 `p->next = s`。",
        "如果顺序反过来，`p->next` 已经指向 s，原来的 30 入口丢失。",
        "删除 20 时保存待删结点，再让前驱越过它，最后释放内存。每一次赋值都要能在图上画出来。"
      ],
      checkpoints: ["指出已知前驱插入为什么是 O(1)", "解释找前驱为什么通常是 O(n)", "画出删除首元结点的特殊情况", "说明释放内存前必须保留哪个指针"],
      misconceptions: ["把结点值相邻当成内存相邻", "插入赋值顺序写反", "删除后继续访问已释放结点", "忽略空链表和单结点链表"],
      exercises: ["手推头插、尾插、按值删除", "写出逆置链表的三指针含义", "让 LLM 生成链表删除函数并设计首结点删除测试"],
      quiz: ["链表插入为什么要先写 `s->next`？", "已知前驱插入的复杂度是多少？", "删除头结点后的 head 应该指向哪里？", "逆置链表至少需要保存哪些临时指针？", "如何用测试发现断链？"]
    },
    4: {
      focus: "括号匹配中栈顶保存最近未匹配左括号",
      outcome: "能用栈状态解释 LIFO，并处理空栈、类型不匹配和最终非空",
      deepDive: [
        "扫描 `a[(b+c)*d]`，遇到 `[` 和 `(` 入栈，遇到 `)` 必须匹配栈顶 `(`。",
        "若输入为 `([)]`，读到 `)` 时栈顶是 `[`，类型不匹配，应立即失败。",
        "扫描结束后栈必须为空；否则说明还有未匹配的左括号。"
      ],
      checkpoints: ["写出 push 前的满栈判断", "写出 pop 前的空栈判断", "解释为什么右括号匹配栈顶而不是栈底", "给出 `([)]` 的栈状态"],
      misconceptions: ["出栈前不判空", "只统计左右括号数量，不检查顺序", "把栈顶下标和元素个数混淆", "结束时不检查栈是否为空"],
      exercises: ["手推 `([]{})` 和 `([)]`", "扩展到引号内括号忽略", "让 LLM 生成括号匹配并补空栈测试"],
      quiz: ["表达式 `([)]` 在哪一步失败？", "空栈时读到右括号应返回什么？", "栈顶变量表示下一个空位还是最后一个元素？", "函数调用栈和显式栈有什么共同点？", "如何测试最终栈非空的情况？"]
    },
    5: {
      focus: "循环队列 `front/rear` 取模与队满判定",
      outcome: "能在圆环上手推入队出队，并说明牺牲一个单元的队满条件",
      deepDive: [
        "容量 6 的循环队列先入队 10,20,30,40，再出队两次，front 从 0 移到 2。",
        "继续入队 50,60 后 rear 从 5 回到 0；再入队 70 使用释放出来的 0 号槽。",
        "如果 `(rear + 1) % capacity == front`，说明再写就会覆盖队头，因此判满。"
      ],
      checkpoints: ["标出 front 指向队头还是空位", "标出 rear 指向下一个可写位置", "解释 `front == rear` 为什么表示空", "给出队满公式"],
      misconceptions: ["把循环队列画成直线", "队满仍用 `front == rear`", "入队后忘记取模", "出队时搬移数组元素"],
      exercises: ["手推 8 次入队/出队混合操作", "比较牺牲一个单元和增加 size 字段两种方案", "设计队满后继续入队测试"],
      quiz: ["容量 6、front=2、rear=1 时队列是否满？", "rear 指向什么位置？", "为什么出队不用移动元素？", "如何区分队空和队满？", "层序遍历为什么需要队列？"]
    },
    6: {
      focus: "KMP 的 `next` 数组和文本指针不回退",
      outcome: "能构造一个模式串的 `next` 表，并解释失配时 `j` 如何回退",
      deepDive: [
        "以模式串 `ababaca` 为例，最长相等前后缀长度表为 `[0,0,1,2,3,0,1]`。",
        "当已匹配 `ababa` 后在 `c` 处失配，`j` 不回到 0，而是根据 `next[j-1]` 回到 3。",
        "文本指针 `i` 不回退，是 KMP 从 O(nm) 降到 O(n+m) 的关键。"
      ],
      checkpoints: ["手算 `ababaca` 的 next 表", "说明 `i` 不回退的原因", "比较朴素匹配和 KMP 的失败路径", "给出模式串为空时的约定"],
      misconceptions: ["把 next 当成移动距离而不是前后缀长度", "失配后让文本指针回退", "构造 next 时混淆前缀和后缀", "没有处理空模式串"],
      exercises: ["构造 `abcac` 和 `ababaca` 的 next", "手推文本 `ababcabcacbab` 中查找 `abcac`", "让 LLM 解释 KMP 并要求它列出每次失配的 `i/j`"],
      quiz: ["`ababaca` 的 next 数组是什么？", "KMP 失配时哪个指针不回退？", "朴素匹配最坏输入如何构造？", "next[j-1] 表示什么？", "如何测试模式串为空？"]
    },
    7: {
      focus: "二叉树遍历的递归调用栈",
      outcome: "能写出前中后序遍历序列，并解释空子树递归出口",
      deepDive: [
        "对树 A(B(D,E),C(NULL,F))，前序为 A B D E C F，中序为 D B E A C F，后序为 D E B F C A。",
        "同一棵树只改变访问根的位置，就得到三种遍历。",
        "每个空孩子都对应一次递归出口；没有出口就会解引用空指针。"
      ],
      checkpoints: ["写出空树高度的定义", "区分访问根和进入子树", "画出递归调用栈", "说明层序遍历为什么用队列"],
      misconceptions: ["认为空指针不是结构的一部分", "只背遍历口诀，不知道递归栈", "混淆树高和结点数", "由单一遍历序列误判唯一结构"],
      exercises: ["给定前序和中序重建小树", "写出递归版和非递归版前序遍历", "设计空树和单结点树测试"],
      quiz: ["树 A(B(D,E),C) 的前序是什么？", "递归遍历的出口是什么？", "层序遍历使用什么辅助结构？", "树高和结点数有什么区别？", "为什么只有前序不能唯一确定二叉树？"]
    },
    8: {
      focus: "AVL 四种失衡类型和旋转后中序不变",
      outcome: "能判断 LL/RR/LR/RL，并证明旋转恢复平衡但不破坏 BST 有序性",
      deepDive: [
        "插入 30,20,10 是 LL 型，对 30 右旋，结果为 20 根、10 左、30 右。",
        "插入 30,10,20 是 LR 型，先对 10 左旋，再对 30 右旋。",
        "旋转前后中序序列都保持为 10,20,30；改变的是局部高度和父子关系。"
      ],
      checkpoints: ["判断插入路径属于 LL/RR/LR/RL", "写出右旋涉及的三个子树", "说明普通 BST 最坏 O(n) 的原因", "说明 AVL 为什么保证 O(log n)"],
      misconceptions: ["只记旋转名字，不看插入路径", "旋转后破坏 BST 中序有序", "忘记更新高度", "把普通 BST 当成总是 O(log n)"],
      exercises: ["分别手推 LL、RR、LR、RL 最小例子", "删除双孩子结点并说明替代策略", "让 LLM 生成 AVL 插入并检查高度更新"],
      quiz: ["插入 30,10,20 是哪种失衡？", "右旋后中序序列是否改变？", "普通 BST 为什么可能退化成链表？", "AVL 旋转本身复杂度是多少？", "高度字段什么时候更新？"]
    },
    9: {
      focus: "堆的数组下标关系与上滤/下滤",
      outcome: "能在数组和完全二叉树之间转换，并手推 push/pop 的修复路径",
      deepDive: [
        "数组下标 i 的父结点是 `(i-1)/2`，左右孩子是 `2*i+1` 和 `2*i+2`。",
        "最小堆插入 3 时先放末尾，再和父结点比较，逐层上滤到正确位置。",
        "pop 时用末尾元素补根，再下滤，必须在左右孩子中选择更小者。"
      ],
      checkpoints: ["由下标画出完全二叉树", "说明堆只保证根最小，不保证数组整体有序", "手推一次下滤", "解释建堆 O(n) 的直观原因"],
      misconceptions: ["把堆数组看成有序数组", "下滤只比较左孩子", "pop 后忘记缩小 size", "混淆大根堆和小根堆"],
      exercises: ["手推插入序列 7,12,3,18,5", "实现 `heap_pop` 的空堆返回", "比较堆和 BST 的查找能力"],
      quiz: ["下标 6 的父结点下标是多少？", "最小堆数组一定递增吗？", "pop 后为什么要下滤？", "建堆从哪个结点开始？", "堆适合解决什么类型的问题？"]
    },
    10: {
      focus: "DFS/BFS 中 `visited` 的时机和遍历辅助结构",
      outcome: "能用邻接表手推 DFS/BFS，并解释何时标记 visited",
      deepDive: [
        "BFS 从 A 出发时，发现 B/C 就立刻标记并入队，而不是等出队后才标记。",
        "DFS 沿一条路径深入，隐含结构是递归栈；BFS 按层扩展，显式结构是队列。",
        "非连通图需要从每个未访问顶点重新启动遍历。"
      ],
      checkpoints: ["画出邻接矩阵和邻接表的空间差异", "说明 BFS 入队即标记的原因", "给出非连通图遍历策略", "比较 DFS/BFS 的访问顺序"],
      misconceptions: ["visited 标记太晚导致重复入队", "把邻接矩阵和邻接表复杂度混为一谈", "忘记处理非连通图", "递归 DFS 不设出口"],
      exercises: ["手推一张 5 顶点图的 DFS/BFS", "设计有环图测试", "让 LLM 生成 BFS 并检查标记时机"],
      quiz: ["BFS 使用什么辅助结构？", "为什么发现顶点时就标记 visited？", "邻接表遍历复杂度是多少？", "非连通图如何遍历所有顶点？", "DFS 的隐含栈在哪里？"]
    },
    11: {
      focus: "Dijkstra 的 `dist/prev/settled` 表格",
      outcome: "能逐轮填写 Dijkstra 状态表，并说明非负权前提",
      deepDive: [
        "从 A 出发，初始化 `dist[A]=0`，其余为无穷大，`prev` 全为空。",
        "每轮选择未确定顶点中 dist 最小者，然后松弛它的出边；进入 settled 后距离不再改变。",
        "如果存在负权边，当前最小 dist 的贪心选择可能被后续路径推翻，所以 Dijkstra 不适用。"
      ],
      checkpoints: ["填写一轮 dist/prev/settled 表", "解释松弛条件 `dist[u]+w < dist[v]`", "说明 Dijkstra 的非负权前提", "比较 O(V^2) 和 O(E log V) 实现差异"],
      misconceptions: ["把 visited 当成 BFS 的 visited，而不是已确定集合", "忘记不可达顶点保持 INF", "对负权边使用 Dijkstra", "只背步骤，不知道 dist 含义"],
      exercises: ["给定 5 顶点图手算最短路表", "构造一个负边反例说明 Dijkstra 失败", "让 LLM 输出 Dijkstra 表并核查每次松弛"],
      quiz: ["Dijkstra 每轮选择什么顶点？", "松弛操作的条件是什么？", "为什么负权边会破坏算法？", "`prev` 数组有什么用？", "邻接矩阵版本复杂度是多少？"]
    },
    12: {
      focus: "顺序、二分、哈希三种定位路径的前提",
      outcome: "能说明每种查找结构的前提、失败路径和最坏情况",
      deepDive: [
        "顺序查找不要求有序，但最坏要看完所有元素。",
        "二分查找依赖有序数组和随机访问，每次比较后排除一半。",
        "哈希查找依赖哈希函数和冲突处理；线性探测查找失败必须走到空槽，删除槽不能直接当空槽。"
      ],
      checkpoints: ["说明二分查找的循环不变量", "手推哈希冲突后的探测序列", "解释装载因子对性能的影响", "区分查找成功和失败的探测路径"],
      misconceptions: ["对无序数组使用二分", "哈希表平均 O(1) 就忽略最坏情况", "删除槽直接置空导致探测链断裂", "哈希函数和冲突策略不一致"],
      exercises: ["手推 key%11 的线性探测插入", "构造二分查找失败例子", "让 LLM 解释哈希删除并检查删除标记"],
      quiz: ["二分查找的前提是什么？", "线性探测遇删除槽能否停止？", "装载因子升高会带来什么问题？", "哈希查找失败何时停止？", "顺序查找有什么优势？"]
    },
    13: {
      focus: "简单排序的比较、移动和稳定性",
      outcome: "能手推插入/选择/冒泡，并比较稳定性和最好/最坏情况",
      deepDive: [
        "插入排序维护有序前缀，移动比 key 大的元素，近乎有序时很快。",
        "选择排序每轮找最小值，比较次数固定多，但交换次数少。",
        "冒泡排序通过相邻交换把最大值推到后面；若一趟无交换可提前结束。"
      ],
      checkpoints: ["写出插入排序内层循环不变量", "判断三种排序稳定性", "构造逆序最坏输入", "解释冒泡提前终止的条件"],
      misconceptions: ["把选择排序误判为稳定", "插入排序写成频繁交换而不理解移动", "冒泡 swapped 不重置", "只比较时间不比较移动次数"],
      exercises: ["同一数组分别手推三种排序前两轮", "用带编号相等关键字判断稳定性", "让 LLM 给出排序代码并补重复关键字测试"],
      quiz: ["插入排序适合什么输入？", "选择排序为什么移动次数少？", "冒泡排序最好情况如何达到 O(n)？", "稳定性如何用相等关键字测试？", "逆序输入对插入排序意味着什么？"]
    },
    14: {
      focus: "快速排序 partition 的单次 O(n) 与整体递归树",
      outcome: "能区分单次划分和整体排序复杂度，并比较快排、归并、堆排取舍",
      deepDive: [
        "partition 对一个长度为 n 的区间只扫描一次，因此单次代价是 O(n)。",
        "若每次划分接近均衡，递归树高度约为 log n，每层总扫描 O(n)，整体平均 O(n log n)。",
        "若 pivot 总是最小或最大，递归高度退化到 n，整体最坏 O(n^2)。归并稳定但要 O(n) 辅助空间，堆排 O(1) 额外空间但通常不稳定。"
      ],
      checkpoints: ["写出 partition 循环不变量", "区分单次 partition 和整体 quicksort", "构造快排最坏输入", "比较三种高效排序的稳定性和空间"],
      misconceptions: ["把 partition 写成 O(n log n)", "忽略重复关键字导致极端不平衡", "把归并的单次合并和整体复杂度混淆", "认为堆排后堆数组一开始就是有序的"],
      exercises: ["手推 Lomuto 或 Hoare partition", "画出均衡和退化两棵递归树", "让 LLM 解释快排复杂度并检查是否混淆单次/整体"],
      quiz: ["单次 partition 的复杂度是多少？", "快排平均 O(n log n) 来自哪两部分？", "什么输入会让快排退化？", "归并排序的额外空间是多少？", "堆排序是否稳定？"]
    },
    15: {
      focus: "线性排序的前提：范围、位数和稳定性",
      outcome: "能说明 `n/k/d/r` 的含义，并判断何时不该使用线性排序",
      deepDive: [
        "计数排序对关键字范围 0..k 建立计数数组，代价是 O(n+k)。如果 k 远大于 n，空间和初始化成本会很高。",
        "稳定计数排序要先做前缀和，再从右向左扫描输入放入输出数组。",
        "基数排序每一位都要用稳定排序；若某一轮不稳定，前面位次的相对顺序会被破坏。"
      ],
      checkpoints: ["说明 k、d、r 分别表示什么", "手推计数数组和前缀和", "解释稳定分配为什么从右向左", "判断负数或大范围关键字如何处理"],
      misconceptions: ["认为线性排序适用于所有数据", "忽略关键字范围导致空间爆炸", "基数排序每轮使用不稳定排序", "计数排序不处理偏移"],
      exercises: ["手推输入 [4,2,2,8,3,3,1] 的计数排序", "设计含负数的范围压缩方案", "让 LLM 生成计数排序并检查稳定性"],
      quiz: ["计数排序复杂度中的 k 是什么？", "为什么稳定计数排序从右向左扫描？", "基数排序为什么要求每轮稳定？", "关键字范围很大时有什么问题？", "桶排序的期望复杂度依赖什么前提？"]
    },
    16: {
      focus: "学生记录系统中顺序表、哈希索引和排名视图的一致性",
      outcome: "能为一个小系统写出对象-操作-结构选择表，并说明每个操作维护哪些结构",
      deepDive: [
        "学生记录系统可以用顺序表保存记录，用哈希表从学号定位顺序表下标，用排序生成成绩排名。",
        "插入时必须同时写顺序表和哈希索引；删除中间记录后，顺序表后续元素下标变化，索引必须重建或修正。",
        "综合设计不是问某个结构会不会写，而是问高频操作、结构职责、复杂度和一致性维护是否匹配。"
      ],
      checkpoints: ["写出对象-操作表", "列出每个操作要更新的结构", "说明删除后索引如何保持一致", "给出 public test 之外的 hidden case"],
      misconceptions: ["把多个结构堆在一起但不说明职责", "删除记录后不更新索引", "只报告平均复杂度不说明装载因子", "把 LLM 生成代码当成系统设计"],
      exercises: ["设计学生记录系统的 ADT 头文件", "列出 add/delete/find/rank 的复杂度矩阵", "让 LLM 生成一个删除实现并设计能暴露索引不同步的测试"],
      quiz: ["学生系统中哈希表保存什么？", "删除顺序表中间元素后索引为什么会失效？", "排名视图可以何时生成？", "综合设计最重要的不变量是什么？", "AI 生成系统代码后第一组 hidden test 应测什么？"]
    }
  };
  return data[week.id] || data[1];
}

function weeklyWorkedExample(week) {
  const data = weeklyTeachingData(week);
  return `
## 本周完整推导例题

教师可以把下面例题作为课堂主线，学生应能在纸上复现。

${numberedList(data.deepDive)}
`;
}

function advancedDiagnosisBlock(week) {
  const blocks = {
    6: [
      "## 高阶诊断：KMP 指针是否回退",
      "",
      "错误实现常把 KMP 写回朴素匹配：失配时执行 `i = i - j + 1; j = 0;`。这能找到答案，但已经失去 KMP 的核心优势。",
      "",
      "课堂上应要求学生记录失配时的三列状态：`i`、`j`、`next[j-1]`。正确 KMP 的关键是文本指针 `i` 不回退，只让模式串指针 `j` 回到可复用的最长相等前后缀位置。",
      "",
    ...
    8: [
      "## 高阶诊断：旋转不是只改根结点",
      "",
      "AVL 旋转最容易写成“形状看起来对了，但高度和子树关系没有维护”。右旋必须保存中间子树、接回父子关系，并按从下到上的顺序更新高度。",
      "",
      "课堂上要求学生用 `30,20,10` 推 LL，用 `30,10,20` 推 LR。每次旋转后检查两件事：中序序列是否仍然有序，所有结点高度是否重新计算。",
      "",
      "可用追问：旋转改变了父子关系，为什么没有改变 BST 的中序序列？"
    ],
    11: [
      "## 高阶诊断：Dijkstra 的非负权前提",
      "",
      "Dijkstra 的 settled 集合表示“最短距离已经确定”。这个结论依赖所有边权非负。可用反例：`A -> B` 权重 2，`A -> C` 权重 5，`C -> B` 权重 -4。",
      "",
      "从 A 出发时，算法可能先确定 B 的距离为 2；但之后通过 C 到 B 的路径长度为 1，会推翻已经 settled 的 B。这说明负权边会破坏“当前最小 dist 已经不可改进”的贪心前提。",
      "",
    ...
    12: [
      "## 高阶诊断：开放定址删除不能简单置空",
      "",
      "设哈希表长度为 11，使用线性探测，依次插入 `12, 23, 34`。它们的初始地址都是 1，会形成连续探测链。",
      "",
      "如果删除 12 时把下标 1 直接置为空，再查找 34，查找会在下标 1 看到空槽后提前停止，错误地认为 34 不存在。正确做法通常是使用删除标记，或删除后重建受影响的探测链。",
      "",
      "课堂上要区分三种槽状态：从未使用、已删除、正在使用。查找失败只能在遇到“从未使用”的槽时确定。"
    ],
    14: [
      "## 高阶诊断：partition 的循环不变量",
      "",
      "快速排序的难点不在递归框架，而在 partition 是否维护了清晰的不变量。大量重复关键字能暴露指针不推进的问题。",
      "",
      "当左右指针都停在等于 pivot 的元素上，如果交换后没有推进，数组不变、指针不变，循环可能重复同一状态。课堂上应让学生用 `[5,5,5]` 或 `[3,5,5,2]` 手推。",
      "",
    ...
    16: [
      "## 高阶诊断：多结构一致性",
      "",
      "综合系统的错误往往不是某个结构单独写错，而是多个结构没有同步。学生记录系统可以用顺序表保存记录、哈希表保存学号到下标的索引、排序视图生成排名。",
      "",
      "建议课堂用操作序列做一致性检查：插入 `202401 Zhang 88`、`202402 Lin 91`、`202403 Wang 95`；删除 `202402`；再查找 `202403`，检查哈希索引是否仍能定位到 `Wang`。",
      "",
      "学生应列出每个操作要维护的结构清单：记录表、学号索引、排名视图、长度字段和失败返回。综合设计的核心是让所有派生结构在操作后重新一致。"
    ]
  };
  return blocks[week.id] ? blocks[week.id].join("\n") : "";
}
...
function llmEraReflection(week) {
  const reflections = {
    1: "大模型很容易给出一个复杂度结论，但它经常省略输入规模、基本操作和最好/最坏前提。本章训练的不是背出 O 记号，而是能审查模型的复杂度解释是否有推导依据。学生学会数循环次数、写递推式和区分增长阶，才有能力判断模型说的“高效”是否真实。",
    2: "大模型能快速生成顺序表代码，但最常见的错误正是本章的核心：`size` 和 `capacity` 混淆、插入移动方向写反、扩容失败后结构被破坏。学习顺序表，是为了看懂连续存储为什么带来 O(1) 下标访问，也为什么让中间插入删除付出搬移代价。只有理解这些不变量，才能审查模型生成的数组代码是否可靠。",
    3: "链表代码看起来短，但大模型很容易在指针赋值顺序、删除首元结点、释放内存上出错。本章的价值是训练学生把每一次 `next` 修改画成结构变化，而不是只看最终输出。能判断“先接后继，再改前驱”的原因，才能发现模型代码里的断链、丢链和内存泄漏。",
    4: "大模型能写出括号匹配的标准答案，但它可能只覆盖正常样例，漏掉空栈、类型交叉和最终非空等失败路径。本章让学生理解栈保存的是“最近未完成任务”，这个不变量比代码模板更重要。掌握这个不变量后，学生才能审查模型是否把 LIFO 用在了正确的问题上。",
    5: "队列代码常被大模型写得像数组下标练习，却容易混淆 `front`、`rear`、空队和满队。本章要学的是 FIFO 秩序如何通过循环数组或链式结构稳定保存。理解队列后，学生才能判断模型生成的 BFS、任务调度或缓冲区代码是否会重复处理、漏处理或越界。",
    6: "KMP 是最容易被大模型“背代码式”回答的主题之一：代码可能能跑样例，却说不清 `next` 的含义，也可能在失配时错误地让文本指针回退。本章训练学生追踪 `i/j/next` 的状态变化，理解已匹配信息如何被复用。这样才能判断模型解释的是 KMP，还是披着 KMP 外壳的朴素匹配。",
    7: "大模型可以直接输出遍历序列，但学生如果不理解递归调用栈，就很难发现空树、单结点树和退化树上的错误。本章让学生把树看成递归结构：每个结点都有左右子树，每个操作都要处理空子树出口。理解这点后，学生才能审查模型生成的树代码是否会空指针访问、遍历顺序混乱或重建前提不足。",
    8: "平衡树代码是大模型最容易生成“看起来专业但细节危险”的内容：旋转漏接子树、忘记更新高度、删除后不再平衡都很常见。本章不是要求学生机械记住四种旋转，而是理解旋转为什么保持 BST 中序有序、又如何控制树高。具备这个判断后，学生才能审查模型写出的 AVL 实现是否真的维护了平衡。",
    9: "大模型常把堆误解释成“有序数组”，或者用完整排序来实现优先队列，导致复杂度不符合需求。本章让学生理解堆只维护局部父子关系，却能高效支持取最小/最大。掌握堆序和完全二叉树数组映射后，学生才能判断模型的 `push/pop` 是否真的只做了上滤和下滤。",
    10: "图遍历代码由大模型生成并不难，但它常漏掉非连通图、重复入队、visited 标记时机和邻接表示选择。本章让学生理解图算法是在关系网络中传播状态，`visited` 不是装饰变量，而是防止重复和死循环的核心不变量。只有理解表示和状态传播，才能审查模型生成的 DFS/BFS 是否适用于真实图。",
    11: "大模型很容易把 Dijkstra、Prim、拓扑排序当成固定模板套用，却忽略非负权、连通性、有向无环等前提。本章训练学生逐轮维护 `dist/prev/settled` 或入度状态，知道每一步为什么成立。这样才能发现模型把 Dijkstra 用到负权图、把拓扑排序用到有环图这类根本性错误。",
    12: "模型能生成哈希表代码，但开放定址中的删除标记、探测链、装载因子和失败查找路径很容易被写错。本章让学生理解“快速查找”依赖前提：有序、映射、负载控制或树高控制。掌握这些前提后，学生才能判断模型给出的平均 O(1) 是否只是口号，还是有冲突处理和边界维护支撑。",
    13: "简单排序很容易被模型写对表面流程，却说不清比较次数、移动次数和稳定性的差别。本章的价值在于让学生逐步观察元素如何移动，尤其是重复关键字的相对次序。理解这些细节后，学生才能审查模型关于“稳定”“最好情况”“提前终止”的说法是否准确。",
    14: "高效排序是大模型最容易讲得过于笼统的章节：它可能只说快排平均 O(n log n)，却不说明 partition 不变量和最坏退化；也可能混淆归并、堆排的空间与稳定性。本章训练学生把单次划分、递归树和实现取舍分开看。这样才能判断模型推荐的排序算法是否匹配输入分布和工程约束。",
    15: "大模型常把计数排序、桶排序、基数排序概括为“线性时间更快”，却忽略关键字范围、位数、分布和稳定性前提。本章让学生理解线性排序不是通用替代品，而是利用额外信息把比较问题转化为计数和分配。掌握这些前提后，学生才能判断模型什么时候在过度推荐线性排序。",
    16: "综合设计中，大模型最容易给出一堆结构名，却没有说明每个结构的职责和同步关系。本章要求学生把对象、操作、结构选择、复杂度和一致性维护放到同一张表里。能判断插入、删除、查询、排名分别要更新哪些结构，学生才不会把模型生成的“能跑代码”误当成可靠系统设计。"
  };
  return reflections[week.id] || "大模型时代，学习数据结构的重点不是背代码，而是审查结构不变量、边界条件、复杂度前提和测试证据。";
}
...
${weeklyWorkedExample(week)}

${advancedDiagnosisBlock(week)}
...
${bulletList(weeklyTeachingData(week).misconceptions)}

## 课堂检查点

${bulletList(weeklyTeachingData(week).checkpoints)}
...
function exercisesContent(week) {
  ...
  const data = weeklyTeachingData(week);
  const complexityTitle = week.id === 16 ? "## D. 复杂度矩阵与设计评审（挑战）" : "## D. 复杂度与证明（挑战）";
  const operationLines = week.operations.map((op) => {
    if (week.id === 16) return `    - ${op[0]}：${op[1]}；需要提交 ${op[2]}`;
    return `    - ${op[0]}：${op[2]}，关键动作：${op[1]}`;
  }).join("\n");
  return `
...
${numberedList(data.exercises)}

## G. 设计与 AI 审查（拓展）

14. 设计一个真实应用场景，说明为什么本周结构比更简单结构更合适；必须写出对象、关系、高频操作和不适用场景。
15. 构造一个能暴露常见错误的最小反例，并记录操作中间状态，例如数组移动、指针重连、递归调用栈、探测链或排序区间。
16. 让代码大模型生成本周核心操作的另一种实现。你需要提交：prompt、模型输出摘要、一个不变量风险、一个边界测试和修正说明。
17. 给出一个小型性能或复杂度证据：比较次数、移动次数、探测次数、递归深度或辅助空间，并说明复杂度结论依赖的前提。
`;
}
...
function answersContent(week) {
  const diag = diagnosticSnippetFor(week);
  const data = weeklyTeachingData(week);
  const complexityReference = week.id === 16
    ? week.operations.map((op) => `- ${op[0]}：${op[1]}。合格答案应形成 ${op[2]}，并说明它如何约束系统正确性。`).join("\n")
...
${bulletList(data.exercises)}

参考答案不要求形式一致，但必须体现本周核心抓手：${data.focus}。

## G. 设计与 AI 审查参考

真实应用场景必须匹配操作频率、输入规模和数据分布。如果问题大量执行本周结构擅长的操作，就有选择理由；若只是一次性扫描，普通数组或直接循环可能更简单。

评价 LLM 输出时要看五点：接口是否符合题目、不变量是否保持、边界是否完整、复杂度前提是否真实、测试是否能暴露错误。优秀答案应包含反例或状态表，而不是只说“运行通过”。头文件分离版本应保持结构定义、函数声明、实现和 main 的职责边界。
...
- GitHub Pages 入口：[https://fjiangai.github.io/data_structure_lesson/onlineweb/](https://fjiangai.github.io/data_structure_lesson/onlineweb/)
...
公开 Pages 站点面向学习和自测，不承担正式考试评分。正式考试、标准答案和真实 hidden tests 建议放在教师私有仓库或学校教学平台中维护，公开仓库只保留模拟卷、练习题和可运行的 public tests。详细边界见 [ASSESSMENT_SECURITY.md](ASSESSMENT_SECURITY.md)。
...
- \`teacher_guide/\` 提供课堂组织建议、评分 Rubric、16 周逐周教案和高阶教学深化指南，便于教师和助教使用。
...
3. 完成练习后对照参考答案自查；在线考试页面用于自测，不作为正式成绩依据。
4. 用 \`interactive.html\` 做课堂演示或学生自学演示。
5. 使用 \`test/\` 中的测验和作业组织随堂测试、课后作业或复习。
6. 使用 \`assignments/\` 中的实验模板组织阶段性编程作业。
7. 使用 \`teacher_guide/\` 中的 Rubric 统一助教批改标准；需要提高挑战度时，使用 [teacher_guide/deep_teaching_guide.md](teacher_guide/deep_teaching_guide.md) 设计不变量审查、反例构造、性能实验和 AI 代码审查任务。
...
5. 打开 [https://fjiangai.github.io/data_structure_lesson/onlineweb/](https://fjiangai.github.io/data_structure_lesson/onlineweb/)。
...
${numberedList(weeklyTeachingData(w).quiz)}
`).join("\n");
  return `
# 随堂测试题
...
<body>
  <header class="exam-header">
    <nav>
      ...
      <a href="viewer.html?src=../ASSESSMENT_SECURITY.md">考核边界</a>
    </nav>
    <h1>数据结构在线自测考试</h1>
    <p>本页面是公开静态站点中的自测工具，提交后可查看分数、错题和解析。题目与答案位于前端文件中，不作为正式课程成绩依据。</p>
  </header>
  <main>
    <section class="exam-notice">
      <strong>自测定位</strong>
      <span>适合复习诊断和即时反馈；正式考试、hidden tests 与成绩评定请使用教师私有仓库或学校教学平台。</span>
    </section>
...
a:focus-visible, button:focus-visible, input:focus-visible, select:focus-visible { outline:3px solid rgba(20,122,99,.35); outline-offset:3px; }
...
.exam-toolbar, .exam-meta, .question-card, .exam-result, .exam-notice { background:var(--paper); border:1px solid var(--line); border-radius:8px; padding:16px; }
.exam-notice { display:grid; gap:4px; margin-bottom:14px; border-color:#f2c37b; background:#fff8ed; }
.exam-notice strong { color:var(--rust); }
.exam-notice span { color:var(--muted); }
...
<body>
  ...
  <main id="mainContent">
    <section class="band summary">
      ...
      <article class="progress-tools">
        <strong>学习记录</strong>
        <span id="progressStatus">保存在本地浏览器，可导出备份</span>
        <div>
          <button id="exportProgress" type="button">导出</button>
          <button id="importProgress" type="button">导入</button>
    ...
    <section class="content-section resource-center">
      ...
      <div class="resource-grid">
        ...
        <a href="viewer.html?src=../ASSESSMENT_SECURITY.md"><strong>考核边界</strong><span>公开自测、正式考试与 hidden tests 分层</span></a>
        <a href="viewer.html?src=../teacher_guide/README.md"><strong>教师指南</strong><span>课堂组织与评分 Rubric</span></a>
        <a href="viewer.html?src=../teacher_guide/deep_teaching_guide.md"><strong>高阶教学</strong><span>不变量审查、反例构造、性能实验与 AI 代码审查</span></a>
...
a:focus-visible, button:focus-visible, input:focus-visible, select:focus-visible {
  outline: 3px solid rgba(31, 138, 112, .35);
  outline-offset: 3px;
}
.site-header {
  background:
...
.progress-tools { display: grid; gap: 8px; }
.progress-tools div { display: flex; gap: 8px; flex-wrap: wrap; }
...
.progress-tools button + button { background: #fff; color: var(--green-dark); }
.progress-tools .bad { color: #b91c1c; }
...
.custom-lab-panel p.is-error { color: #b91c1c; font-weight: 800; }
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Run lab starter public tests
        run: python3 tools/run_lab_tests.py --all --expect-starter-fail

      - name: Run lab reference public tests
        run: python3 tools/run_reference_solutions.py
...
function runReferenceSolutionsScript() {
  return String.raw`
import json
import subprocess
import sys
...
function sanitizeExamplesScript() {
  return String.raw`
import argparse
import os
import platform
...
function deepTeachingGuide() {
  return `
# 高阶教学深化指南

本指南用于把课程从“知识点覆盖完整”提升到“能训练结构选择、反例构造、实验解释和代码审查能力”。每周至少选择一种高阶训练嵌入课堂或作业。

## 训练目标

1. 说明为什么选择某种结构，而不是只实现指定结构。
2. 找出空、满、重复关键字、删除、内存释放、探测链断裂、负权边等边界风险。
3. 说清最好、最坏、平均、摊还、数据范围、关键字分布和实现方式等复杂度前提。
4. 用手推过程、单元测试、反例测试、性能数据和代码审查证据证明实现可信。

## 四类高阶任务

### 不变量审查

学生提交结构体字段含义、核心操作前后的不变量、失败路径和能检查不变量的测试或状态图。

### 反例构造

学生提交能击穿错误结论或错误实现的最小输入、错误发生的中间状态和修正说明。

### 性能实验解释

学生至少比较普通、极端、退化三类输入，记录比较次数、移动次数、探测次数、递归深度或运行时间，并解释一个看起来反常的结果。

### AI 生成代码审查

学生提交原始 prompt、模型输出中的一个合理点和一个风险点、自行设计的边界测试以及修正后的说明。

## 课堂组织方式

建议采用“5 分钟反例 + 10 分钟审查 + 5 分钟修正”的短循环：教师给出看似正确的代码或复杂度结论，学生独立构造反例，小组交换状态图，最后回到本周结构不变量和复杂度前提。
`;
}
...
function teacherLessonPlanContent(week, prev, next) {
  const stage = structureStage(week);
  const data = weeklyTeachingData(week);
  const operations = week.operations.map((op, i) => operationTeachingLine(week, op, i)).join("\n");
  const board = [
...
- 选 1 个核心任务让学生课堂手推，建议选择：${data.focus}。
...
${bulletList(data.misconceptions)}

## 课堂活动

- 3 分钟：学生独立画出 5 个元素的小结构图。
- 5 分钟：两人一组互相检查字段含义。
- 8 分钟：全班一起推演 ${data.focus}。
...
function hiddenCaseSet(lab) {
  const cases = {
    1: [
      ["empty-seq", "顺序表为空时删除下标 0", "返回失败且 size 保持 0"],
      ["full-insert", "容量已满时继续插入", "返回失败且原有元素顺序不变"],
      ["head-delete", "链表删除首元结点", "head 正确越过目标结点并释放内存"],
      ["duplicate-find", "插入重复值后查找", "能说明返回第一个匹配或全部匹配的策略"],
      ["middle-insert", "顺序表在中间插入", "只移动必要后缀，不覆盖原值"]
    ],
    2: [
      ["empty-pop", "空栈 pop", "返回失败，不访问非法栈顶"],
      ["mismatch-bracket", "表达式 ([)]", "判定不匹配"],
      ["queue-wrap", "循环队列多次入队出队后回绕", "front/rear 取模正确"],
      ["full-queue", "循环队列满后继续入队", "返回失败且不覆盖 front"],
      ["kmp-overlap", "文本 aaaaa，模式 aaa", "能处理重叠匹配或说明返回第一个匹配"]
    ],
    3: [
      ["single-node", "单结点树遍历", "三种遍历均只输出一个结点"],
      ["empty-tree", "空树高度与遍历", "高度和遍历定义清楚，不解引用 NULL"],
      ["avl-ll", "插入 30,20,10", "触发右旋并保持中序有序"],
      ["avl-lr", "插入 30,10,20", "触发左右旋"],
      ["heap-duplicates", "堆中有重复关键字", "pop 序列仍非降序"]
    ],
    4: [
      ["disconnected-bfs", "非连通图从 A 遍历", "只访问 A 所在连通分量"],
      ["cycle-dfs", "有环图 DFS", "visited 防止无限递归"],
      ["unreachable-shortest", "Dijkstra 查询不可达顶点", "距离保持 INF"],
      ["negative-edge", "含负权边", "拒绝运行或明确说明 Dijkstra 不适用"],
      ["hash-collision", "多个 key 落入同一桶", "探测/链地址策略一致"]
    ],
    5: [
      ["already-sorted", "输入已升序", "插入/归并/计数保持正确，比较移动成本可解释"],
      ["reverse-sorted", "输入逆序", "简单排序能体现最坏移动"],
      ["duplicates", "存在重复关键字", "说明稳定性或给出稳定输出"],
      ["negative-values", "含负数", "比较排序支持；计数排序需偏移或拒绝"],
      ["single-element", "单元素数组", "不发生越界访问"]
    ],
    6: [
      ["duplicate-id", "重复学号插入", "拒绝插入或更新策略明确"],
      ["delete-reindex", "删除中间记录后继续查找", "哈希索引中的下标同步更新"],
      ["tie-rank", "成绩相同", "排名规则稳定且可解释"],
      ["missing-delete", "删除不存在学号", "返回失败且结构不变"],
      ["rehash-load", "大量插入导致高装载因子", "扩容或冲突策略可证明"]
    ]
  };
  return cases[lab.id].map(([id, trigger, expected]) => ({ id, trigger, expected }));
}
...
def inspect_source(source, enforce_signals=False):
    ...
    if "LAB${String(lab.id).padStart(2, "0")}_STARTER" in text:
...
function assessmentSecurityContent() {
  return `
# 考核安全与公开发布边界
...
function generatorGuideContent() {
  return `
# 课程生成器维护说明
...
function teachingQualityContent() {
  const rows = weeks.map((week) => {
    const data = weeklyTeachingData(week);
    return `| ${weekName(week)} | ${week.shortTitle} | ${week.topics.slice(0, 3).join("、")} | ${data.focus} | ${data.outcome} |`;
  }).join("\n");
  return `
# 教学质量控制与每周达标标准
...
当学生已经能完成基础题时，不建议继续增加零散概念，而应提高任务证据要求：

1. **从答案到判断**：要求学生说明为什么选择某种结构，而不是只实现指定结构。
2. **从通过到可信**：要求学生给出边界测试、反例测试和不变量说明。
3. **从结论到前提**：要求学生区分最好、最坏、平均、摊还和数据分布前提。
4. **从使用 AI 到审查 AI**：要求学生指出模型输出中的不变量维护、边界遗漏和复杂度误判。

每周至少可以布置一个高阶问题：

- 设计一个能击穿错误实现的最小输入。
- 给一段错误代码，要求画出失败时的结构状态。
- 给两种结构选择，要求比较操作频率和复杂度前提。
- 给一组性能数据，要求解释为什么和理论复杂度看起来不一致。
...
function generate() {
  ...
  writeFile("teacher_guide/deep_teaching_guide.md", deepTeachingGuide());
