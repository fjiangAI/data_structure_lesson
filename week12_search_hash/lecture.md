...
1. **二分：检查中点**：mid=3，对应 19。目标 24 更大，因此左边界移动到 mid+1。
2. **二分：缩小区间**：现在只需在下标 4 到 6 中查找。31 大于 24，右边界左移。
3. **二分：找到目标**：mid=4，找到目标。二分的高效来自每轮排除一半。
4. **哈希：线性探测**：查找 46 从 hash(46)=2 开始。2 号槽被 13 占用，继续探测到 3 号槽找到 46。
