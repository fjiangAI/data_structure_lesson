...
本周可以从这个具体问题进入：AVL 插入 30、20、10 后出现 LL 失衡，观察右旋如何恢复高度。

学习时不要先背定义，而要先回答：如果只用最朴素的数组、循环或指针，会在哪个操作上变慢、变乱或容易出错？数据结构的价值就在于把对象之间的关系稳定地组织起来，让操作可以按照可预测的方式执行。

## 结构不变量

AVL 要求每个结点左右子树高度差不超过 1；旋转只改变局部父子关系，不破坏中序有序。
...
1. **插入 30**：第一 个关键字成为根结点。
2. **插入 20**：20 小于 30，插入到左子树。此时仍然平衡。
3. **插入 10 后失衡**：30 的左子树高度比右子树高 2，这是 LL 型失衡。
4. **对 30 右旋**：20 成为新的局部根，10 和 30 分别成为左右孩子，中序序列仍是 10,20,30。
