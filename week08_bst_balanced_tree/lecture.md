...
1. **BST 查找**：按关键字大小选择左右子树。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 平均 O(log n)，最坏 O(n)。
2. **BST 插入**：查找失败位置成为新叶子。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 平均 O(log n)。
3. **BST 删除**：叶子、单孩子、双孩子分别处理。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 平均 O(log n)。
4. **AVL 旋转**：局部改变父子关系恢复高度。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
5. **AVL 插入**：沿递归返回路径更新高度并旋转。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(log n)。
