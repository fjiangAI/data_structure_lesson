...
1. **计数排序**：统计每个关键字出现次数并前缀累加。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n+k)。
2. **桶排序**：按分布把元素放入桶，再桶内排序。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 期望 O(n+k)。
3. **基数排序**：按位稳定排序多轮。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(d(n+r))。
4. **稳定分配**：从后向前放置可保持相等元素顺序。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
5. **范围压缩**：处理 min 到 max 的偏移。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n+k)。
