...
1. **计数排序：统计次数**：计数排序扫描原数组，记录每个关键字出现多少次，适合关键字范围较小的整数。
2. **计数排序：稳定放置**：通过前缀和确定位置，从后向前填入输出数组，可以保持稳定性。
3. **桶排序：按区间分桶**：桶排序按数据分布把元素放进不同区间的桶里，桶内再排序或插入。
4. **基数排序：个位稳定分配**：基数排序按位处理。先按个位稳定分配，再按十位、百位重复。
5. **基数排序：多轮后有序**：每一轮按位排序都必须稳定，低位顺序才能被高位排序正确保留。
