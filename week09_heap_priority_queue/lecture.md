...
1. **push**：插入尾部后上滤。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(log n)。
2. **pop**：根与尾交换后下滤。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(log n)。
3. **peek**：读取根元素。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
4. **buildHeap**：从最后一个非叶结点开始下滤。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
5. **heapSort**：反复取堆顶。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n log n)。
