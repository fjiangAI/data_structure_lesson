...
1. **拓扑排序**：反复删除入度为 0 的顶点。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(V+E)。
2. **Dijkstra**：每轮确定一个最近未确定顶点。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(V^2) 或 O(E log V)。
3. **松弛**：尝试用 u 改进 v 的距离。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
4. **Prim**：每轮选择连接生成树的最小边。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(V^2) 或 O(E log V)。
5. **路径恢复**：pre 数组记录前驱。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(path length)。
