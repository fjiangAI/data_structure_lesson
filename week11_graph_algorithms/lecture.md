...
1. **初始化**：源点 A 的距离为 0，其余未知为无穷大。
2. **确定 A 并松弛**：通过 A 可以到达 B 和 C，更新 dist[B]=10，dist[C]=3。
3. **选择 C**：未确定顶点中 C 最近。经 C 到 B 的距离 3+4=7，比 10 更短。
4. **继续确定最短路**：每次选择当前最小 dist 的未确定顶点，最终得到从 A 到各点的最短距离。
