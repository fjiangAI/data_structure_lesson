...
1. **push**：元素进入栈顶。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
2. **pop**：删除并返回栈顶元素。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
3. **peek**：读取栈顶但不删除。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
4. **isEmpty**：判断 top 是否为空。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
5. **括号匹配**：左括号入栈，右括号匹配栈顶。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
