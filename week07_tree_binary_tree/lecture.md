...
1. **访问根 A**：前序遍历规则是根、左、右。先访问根结点 A。
2. **进入左子树 B**：对左子树重复同样规则，访问 B。
3. **访问 B 的孩子**：左子树处理完 D，再处理右孩子 E。
4. **进入右子树 C**：最后处理 A 的右子树 C，再访问 C 的右孩子 F。
