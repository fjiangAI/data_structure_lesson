...
1. **前序遍历**：根、左、右。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
2. **中序遍历**：左、根、右。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
3. **后序遍历**：左、右、根。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
4. **求高度**：1 + max(leftHeight, rightHeight)。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
5. **层序遍历**：队列保存下一层结点。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
