...
1. **插入排序：取出 key**：已排序前缀是 [29]。插入排序取出 key=10，准备把它插入前缀中的正确位置。
2. **插入排序：移动并插入**：29 > 10，所以 29 后移，10 放到下标 0。插入排序的移动是为了维护有序前缀。
3. **选择排序：选择最小值**：选择排序在未排序区间中找到最小值 3，并与区间首元素交换。它每轮只确定一个最终位置。
4. **冒泡排序：相邻交换**：冒泡排序只比较相邻元素，逆序就交换。一趟扫描后，最大值 37 被推到末尾。
5. **三者对比**：三种算法最终都得到有序序列，但比较次数、移动次数和稳定性不同。
