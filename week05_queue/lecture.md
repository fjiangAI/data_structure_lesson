...
1. **enqueue**：从 rear 位置加入元素。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
2. **dequeue**：从 front 位置取出元素。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
3. **front**：读取队头元素。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
4. **isFull**：(rear + 1) % capacity == front。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(1)。
5. **层序处理**：当前对象出队，其相邻对象入队。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
