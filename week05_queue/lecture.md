...
1. **初始入队**：元素从 rear 写入，rear 每次加 1 并对 capacity 取模。
2. **出队两次**：出队只移动 front，不移动数组中的所有元素，所以是 O(1)。
3. **尾部继续入队**：rear 继续向后写入，空出来的前两个位置暂时不搬移。
4. **rear 回绕**：写到数组末尾后 rear 回到 0，这就是循环队列避免整体搬移的关键。
