...
1. **快速排序划分**：小于枢轴放左侧，大于枢轴放右侧。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 平均 O(n log n)。
2. **归并**：两个有序段线性合并。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n)。
3. **堆排序**：建堆后反复交换堆顶与末尾。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(n log n)。
4. **递归深度**：影响栈空间和最坏情况。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 O(log n) 到 O(n)。
5. **稳定性**：归并可稳定，快排和堆排序通常不稳定。分析时先找会重复执行的语句，再判断重复次数，所以复杂度为 概念判断。
