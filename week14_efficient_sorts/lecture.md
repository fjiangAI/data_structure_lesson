...
1. **快速排序：选择枢轴**：快排选末尾 10 作为 pivot，扫描数组，把不大于 pivot 的元素放到左侧。
2. **快速排序：枢轴归位**：划分结束后，10 的最终位置已经确定，左右两侧再递归排序。
3. **归并排序：两个有序段**：归并排序先把区间拆到足够小，再得到两个有序段：[3,27,38,43] 和 [9,10,82]。
4. **归并排序：线性合并**：合并两个有序段时，每次取较小者放入辅助数组，整个合并过程是 O(n)。
5. **堆排序：堆顶最大值归位**：堆排序先建最大堆，再不断把堆顶最大值交换到末尾，末尾有序区逐步扩大。
